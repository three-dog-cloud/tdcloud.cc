...
const money = ref<number>(props.price)
  ...
  <div class="sm:gap-2 sm:items-center sm:flex sm:flex-wrap">
    <div class="flex items-center justify-between w-full mt-4">
      ...
      <div class="text-sm font-medium text-indigo-600 hover:text-indigo-500 text-size-xs">
        <slot> 默认创建支付渠道，可选其他，注意手续费，悬浮查看手续费详情 </slot>
      </div>
    </div>
    <ToggleGroup
      variant="outline"
          ...
          <Tooltip>
            <TooltipTrigger as-child>
              <img
                :src="item.icon"
                :alt="item.name"
                class="rounded-md object-cover size-6 md:size-8 object-scale-down pa-1"
              />
            </TooltipTrigger>
              ...
              <template v-if="props.oldPayment !== item.uuid">
                ...
                <template v-if="item.handling_fee_percent_max > 0">
                  <p class="flex items-center">
                    手续费比例最高扣除：<span v-if="!props.isShowPrice">{{
                      '￥' + item.handling_fee_percent_max
                    }}</span>
                    <component v-else :is="formatCoinTranslation(item.handling_fee_percent_max)" />
                  </p>
                </template>
                <p v-if="!props.isShowPrice">
                  实际支付:
                  {{
